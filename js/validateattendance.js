...
$(document).on('click','#update_employee_attendance_form #update_employee_attendance',function(e) {
  ...
  $("#loader_9").css("display", "block");
  $("#loader_13").css("display", "block");
  $.ajax({
         type: "POST",
...
$(document).on('click','#update_attendance_form #update_time_log',function(e) {
  ...
  $("#loader_7").css("display", "block");
  $("#loader_11").css("display", "block");
  $.ajax({
         type: "POST",
...
function saveToDB()
{
  ...
  $("#loader_6").css("display", "block");
  $("#loader_8").css("display", "block");
  $("#loader_10").css("display", "block");
  $("#loader_12").css("display", "block");
  var tr ='';
  $.ajax({
...
function CalculateAttendance_TableSummary(attendancetable) {
    try {
       ...
       api.columns(".point_works").each(function (index) {
        ...
        $('.tile_stats_count .total_excess_break').text(excess_break_hour);
      });
