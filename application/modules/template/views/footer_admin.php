                      ...
                      <form id="updateuserform" method="post" enctype="multipart/form-data">
                        ...
                        <p class="hint-text">Update user account.</p>
                         ...
                         <div class="form-group">
                          ...
                          <div id="loader_5" class="center_5"></div>
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                           ...
                           <form class="needs-validation"  id="lead_form" novalidate>
                              ...
                              <div class="form-row">                             
                                ...
                                <div class="form-row">
                                 ...
                                 <div class="form-row">
                                  ...
                                  <div class="col mb-3">
                                     <label for="validationCustom05">Income Level</label>

                                      <select class="form-control income_level" name="income_level" style="height:50px;" >
                                          <option value="VIP">VIP</option>
                                          <option value="UPPER-MIDDLE">UPPER-MIDDLE</option>
                                          <option value="LOWER-MIDDLE">LOWER-MIDLE</option>
                              ...
                              <div id="loader_3" class="center_3"></div>
                      ...
                      <div class="form-row">
                        ...
                        <div class="col mb-3">
                           <label for="validationCustom05">Income Level</label>
                            ...
                            <select class="form-control income_level" name="income_level" style="height:50px;" >
                                <option selected value="">Please Select an Income Level...</option>
                                <option value="VIP">VIP</option>
                                <option value="UPPER-MIDDLE">UPPER-MIDDLE</option>
                                <option value="LOWER-MIDDLE">LOWER-MIDLE</option>
                                <option value="LOW">LOW</option>
...
<script type="text/javascript">
 var pageURL = $(location).attr("href");
  ...
  if (pageURL == 'http://bba.com/crm/dashboard/schedule_attendance'){
     script.src = '<?php echo base_url('js/gijgo.min.js');?>';
 ...
 <script>  
  function setindex_prefix(index_assigned)
    ...
    $('#leadcheaklead_admin').DataTable({
      ...
      $('#allleadadminDatatable').DataTable( {
           "processing": true,
           "serverSide": true,
            "ajax": {
            "url": base_url +  "dashboard/all_lead_admin",
            "type": "POST",
        },
      ...
      var statusleadDatatable =  $('#statusleadDatatable').DataTable( {
           "processing": true,
           "serverSide": true,
            "ajax": {
            "url": base_url +  "dashboard/status_lead",
            "type": "POST",
        },
               ...
               { data: 'contractual_status'},
                ...
                { data: 'date_create',
                    "render" : function( data, type, full ) {
                            // you could prepend a dollar sign before returning, or do it
                            // in the formatNumber method itself
                            var date_create= data == null ? '': data ;
                            return date_create;                          
                          }
               },
                { data: 'lead_date_agent_assign',
...
$('#status_lead_form [name="status_lead"]').on('change', function () {
       statusleadDatatable.columns(13).search($(this).val()).draw() ;//
    ...
    var beggin_date = $('#call_logs_form [name="from_date"]').val();
    var last_date = $('#call_logs_form [name="to_date"]').val();
 ...
 $('#assigndataTable').DataTable({"lengthMenu": [[50, 100, 250, 500, 1000, -1], [50,100, 250, 500, 1000, "All"]], "iDisplayLength": 500});
 ...
 $('#historyleadtable_agent_assign').DataTable();
...
function CalculateTableSummary(otable) {
  ...
  $(function () {
   var start = moment().startOf('month');
    ...
    function cb(start, end) {
        $('#reportrangeleadstatus span').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY'));
    }

    $('#reportrangeleadstatus').daterangepicker({
        startDate: start,
        endDate: end,
        ranges: {
           'Today': [moment(), moment()],
    ...
    $('#assigndatatable').DataTable({"autoWidth": false, columnDefs: [
        ...
        fixedColumns: true});
 $('#assigndatatablevip').DataTable({"autoWidth": false, columnDefs: [
     ...
     $('#leaddataTableselectagent').DataTable(
        ...
        fixedColumns: true});
    $('#leaddataTableselectagentvip').DataTable(
            {"autoWidth": false, columnDefs: [
            { width: '10%', targets: 2 }
     ...
     $('#collectiondataTable').DataTable({"autoWidth": false, columnDefs: [
        ...
        fixedColumns: true});
  $("#addAssignUserform #leaddataTableselectagent #check_all_lead, #updatesubjectform #check_all_lead").click(function () {
     $('#addAssignUserform #leaddataTableselectagent input:checkbox, #updatesubjectform input:checkbox').not(this).prop('checked', this.checked);
 });

  $("#addAssignUserform #leaddataTableselectagentvip #check_all_lead, #updatesubjectform #check_all_lead").click(function () {
     $('#addAssignUserform #leaddataTableselectagentvip input:checkbox, #updatesubjectform input:checkbox').not(this).prop('checked', this.checked);
 });


   $("#addAssignUserform #assigndatatable #check_all_lead, #updatesubjectform #check_all_lead").click(function () {
     $('#addAssignUserform #assigndatatablevip input:checkbox, #updatesubjectform input:checkbox').not(this).prop('checked', this.checked);
 });

  $("#addAssignUserform #assigndatatablevip #check_all_lead, #updatesubjectform #check_all_lead").click(function () {
     $('#addAssignUserform #assigndatatablevip input:checkbox, #updatesubjectform input:checkbox').not(this).prop('checked', this.checked);
 });
...
$('#updatereportform .project_status  option, #updatereportform .interior_designer option, #updatereportform .cover_designer option, #updatereportform .publisher_id option,  #updatereportform .book_color option, #updatereportform .book_size option, #updatereportform .color_type option, #updatereportform .hard_cover_format option, #updatereportform .author_picture option, #updatereportform .book_categories option').each(function () {
   ...
   function hmsToSeconds(s) {
    ...
    var  number_of_days = $('#call_logs_form [name="from_date"]').val() == "" ? 5 : count_date;
       ...
       // var [hours, minutes, seconds] =  typeof sum_hours_estimated === 'string' ? sum_hours_estimated.split(':') : '';
       // var get_minutes = minutes / 60 ;
       // var get_seconds = seconds / 3600 ;
       // // console.log(minutes);
       // // console.log(sum_hours_estimated.split(':').join(","));
       // console.log(hours);

       //  var secsSinceMidnight = (hours*3600) + (minutes*60) + seconds;
       //  var oneTwelth = secsSinceMidnight / numRows;

       //  hours = Math.floor(oneTwelth / 3600);
       //  minutes = Math.floor( (oneTwelth % 3600) / 60);
       //  seconds = Math.floor( (oneTwelth % 3600) % 60);


       // if(hours == '00'){
       //         var get_hour = parseFloat(get_minutes) + parseFloat(get_seconds);
       //         console.log('test');
       // }
       // else{
       //         var get_hour = parseInt(hours) + parseFloat(get_minutes) + parseFloat(get_seconds);
       //          console.log(hours);
       // }

       // var get_hour = parseInt(hours) + parseFloat(get_minutes) + parseFloat(get_seconds);
        // console.log(parseFloat(numRows / get_hour).toFixed(3));
 ...
 function formatPhoneNumber(phoneNumber = '') {
    ...
    $.fn.dataTable.moment('YYYY-MM-DD h:mm A');
     ...
     //      } );
      ...
      var numRows = test_table.rows().count();
      ...
      $('.tile_stats_count .total_call_logs').text(numRows.toLocaleString("en")+'.00');
       ...
       $(function () {
         ...
         $('#call_logs_form [name="from_date"]').val(start.tz('America/New_York').format('YYYY-MM-DD'));
         $('#call_logs_form [name="to_date"]').val(end.tz('America/New_York').format('YYYY-MM-DD'));
         $('#form_report_call_log [name="from_date"]').val(start.tz('America/New_York').format('YYYY/MM/DD'));
         $('#form_report_call_log [name="to_date"]').val(end.tz('America/New_York').format('YYYY/MM/DD'));
...
// JS for call logs
        $('#call_log_reportrange').on('apply.daterangepicker', function(ev, picker) {
               ...
               var start_call_log_report = picker.startDate.format('YYYY/MM/DD');

               var end_call_log_report  = picker.endDate.format('YYYY/MM/DD');


                $('#call_logs_form [name="from_date"]').val(start);
                $('#call_logs_form [name="to_date"]').val(end);
                $('#form_report_call_log [name="from_date"]').val(start_call_log_report);
                $('#form_report_call_log [name="to_date"]').val(end_call_log_report);
...
$('#call_logs_form [name="user_type"]').on('change', function () {
    ...
    $('#form_report_call_log [name="agent_name"]').val(agent_name.split('-')[0]);
...
$(function () {
 ...
 // checklead admin
    ...
    //   var table = $('#leadcheaklead_admin').DataTable({
    //   bInfo:      false,
    //   scrollY:        700,
    //   scrollCollapse: true,
    //   scroller:       true,
    ...
    //   $('div.dataTables_filter').on('keypress', function (e) {
    //       if(e.keyCode == 13)
    //    {
    //       if( $('div.dataTables_filter').val() === '' ) {
